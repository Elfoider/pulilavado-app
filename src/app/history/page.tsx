  ...
  const fetchHistory = async (date: string) => {
    ...
    try {
      ...
      setStats({
        ...
        countCash: cCash, amountCash: aCash,
        countYappy: cYappy, amountYappy: aYappy,
        countCard: cCard, amountCard: aCard,
        countACH: cACH, amountACH: aACH
      });
    ...
    <div className="p-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-white/90 backdrop-blur-sm p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 mt-1">Consulta los cierres de caja de días anteriores.</p>
        </div>
        
        <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200">
            <span className="text-xs font-bold text-gray-400 uppercase px-2">Fecha:</span>
            <input 
              type="date" 
              value={selectedDate}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 rounded-2xl shadow-lg flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">Venta Total</p>
            <DollarSign className="w-5 h-5 text-green-400" />
          </div>
          <p className="text-4xl font-black mt-2">${stats.totalIncome.toFixed(2)}</p>
        </div>

        {/* Total Autos */}
        <div className="bg-white/90 p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
           <div className="flex justify-between items-start">
            <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">Autos Atendidos</p>
            <Car className="w-6 h-6 text-espuma-blue" />
          </div>
          <p className="text-4xl font-black text-gray-800 mt-2">{stats.totalCars}</p>
        </div>
        ...
        <div className="lg:col-span-2 bg-white/90 p-5 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col items-center justify-center p-3 bg-green-50 rounded-xl border border-green-100 text-center">
                ...
                <p className="text-lg font-black text-gray-800">${stats.amountCash.toFixed(2)}</p>
                <p className="text-[10px] text-green-600 font-bold uppercase mt-1">Efectivo ({stats.countCash})</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-3 bg-cyan-50 rounded-xl border border-cyan-100 text-center">
                ...
                <p className="text-lg font-black text-gray-800">${stats.amountYappy.toFixed(2)}</p>
                <p className="text-[10px] text-espuma-blue font-bold uppercase mt-1">Yappy ({stats.countYappy})</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-3 bg-purple-50 rounded-xl border border-purple-100 text-center">
                ...
                <p className="text-lg font-black text-gray-800">${stats.amountCard.toFixed(2)}</p>
                <p className="text-[10px] text-purple-500 font-bold uppercase mt-1">Tarjeta ({stats.countCard})</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-3 bg-gray-100 rounded-xl border border-gray-200 text-center">
                ...
                <p className="text-lg font-black text-gray-800">${stats.amountACH.toFixed(2)}</p>
                <p className="text-[10px] text-gray-500 font-bold uppercase mt-1">ACH ({stats.countACH})</p>
            </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">Sin movimientos.</td></tr>
              ) : (
                services.map((svc) => (
                  <tr key={svc.id} className="hover:bg-blue-50/50 transition">
                    ...
                    <td className="px-6 py-4">
                        <div className="font-bold text-gray-800">{svc.vehicle.model}</div>
                        <div className="text-xs text-gray-500">{svc.vehicle.color}</div>
                    </td>
                    <td className="px-6 py-4">
                        <div className="text-gray-900 font-medium">{svc.clientName || 'General'}</div>
                        <div className="text-xs text-espuma-blue font-bold">{svc.washerName}</div>
                    </td>
                    <td className="px-6 py-4">
      ...
      {selectedService && (
        <ServiceDetailsModal 
            ...
            onClose={() => { setSelectedService(null); fetchHistory(selectedDate); }} 
        />
      )}
    </div>
  );
