...
import { 
  Plus, DollarSign, Trophy, Car, User, Sparkles, AlertCircle 
} from 'lucide-react';
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (washersArray.length > 0) {
        const best = washersArray.reduce((prev, current) => (prev.cars > current.cars) ? prev : current);
        bestName = best.name;
      }
  ...
  return (
    <div className="p-6 space-y-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white/80 backdrop-blur-md p-6 rounded-3xl shadow-sm border border-white/20">
        <div>
          <h1 className="text-3xl font-black text-gray-800 italic tracking-tight">Resumen del Día</h1>
          <p className="text-gray-500 font-medium">Vista general de operaciones</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl border-l-4 border-l-espuma-blue shadow-sm">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Caja (Pagado)</h3>
            <div className="mt-2 flex items-center gap-2">
                <DollarSign className="w-8 h-8 text-espuma-blue" />
                <span className="text-3xl font-black text-gray-800">${stats.totalSales.toFixed(2)}</span>
            </div>
        </div>
        <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl border-l-4 border-l-yellow-500 shadow-sm">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Propinas</h3>
            <div className="mt-2 flex items-center gap-2">
                <Sparkles className="w-8 h-8 text-yellow-500" />
                <span className="text-3xl font-black text-gray-800">${stats.totalTips.toFixed(2)}</span>
            </div>
        </div>
        <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl border-l-4 border-l-green-500 shadow-sm">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Mejor Lavador</h3>
            <div className="mt-2 flex items-center gap-2">
                <Trophy className="w-8 h-8 text-green-500" />
                <span className="text-2xl font-black text-gray-800 truncate">{stats.bestWasherName}</span>
            </div>
        </div>
        <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl border-l-4 border-l-purple-500 shadow-sm">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Autos</h3>
            <div className="mt-2 flex items-center gap-2">
                <Car className="w-8 h-8 text-purple-500" />
                <span className="text-3xl font-black text-gray-800">{stats.totalCars}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {stats.washerStats.map((w) => (
              <div key={w.id} className="bg-white/90 backdrop-blur-sm p-4 rounded-xl shadow-sm border border-white/40 flex flex-col justify-between hover:scale-[1.02] transition-transform">
                  <div>
                      <div className="font-black text-gray-800 text-lg mb-1">{w.name}</div>
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-100 flex justify-between items-end">
                      <div><p className="text-[10px] uppercase font-bold text-gray-400">Propina</p><p className="text-green-600 font-bold">${w.tipsReceived.toFixed(2)}</p></div>
                      <div className="text-right"><p className="text-[10px] uppercase font-bold text-gray-400">Ganado</p><p className="text-espuma-blue font-black text-xl">${w.earnings.toFixed(2)}</p></div>
                  </div>
              </div>
          ))}
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100/50">
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-gray-400">Sin registros hoy.</td></tr>
              ) : (
                services.map((svc) => (
                  <tr key={svc.id} className={`hover:bg-white/60 transition group ${svc.paymentStatus === 'pending' ? 'bg-red-50/50' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                        <div className="font-black text-gray-800">{svc.vehicle?.model}</div>
                        <div className="text-xs text-gray-500 font-bold uppercase">{svc.vehicle?.color}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-gray-500 font-bold">
                        {svc.financials?.tipAmount > 0 
                            ? <span className="text-green-600">+${svc.financials.tipAmount.toFixed(2)}</span> 
                            : 'N/A'}
                    </td>
