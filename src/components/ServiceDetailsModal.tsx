...
import {
  ...
  CreditCard,
  Phone,
} from "lucide-react"; // <--- Nuevo icono CreditCard
...
export default function ServiceDetailsModal({ service, onClose }: Props) {
  ...
  const handleDelete = async () => {
    if (
      confirm(
        "⚠️ ¿Estás seguro de eliminar este servicio? Esto afectará los reportes financieros.",
      )
    ) {
      try {
        setLoading(true);
  ...
  const handleSave = async () => {
    try {
      ...
      const oldRate = service.financials.commissionRate || 0.4;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Encabezado */}
        <div
          className={`p-6 flex justify-between items-start ${isEditing ? "bg-blue-600" : "bg-gray-900"} text-white transition-colors`}
        >
          <div>
              ...
              <div className="flex items flex-col gap-2">
                <div className="flex items-center flex-row gap-2">
                  ...
                  <p className="font-bold text-gray-900">
                    {service.clientName || "Cliente General"}
                  </p>
                </div>
                <div className="flex items-center flex-row gap-2">
                  <Phone className="w-4 h-4 text-gray-400" />
                  <p className="font-bold text-sm text-gray-500">
                    {service.clientPhone || "Cliente General"}
                  </p>
                </div>
              </div>
              <p className="text-xs text-gray-400 mt-1 pl-6">
                Atendido por: {service.washerName}
              </p>
            </div>
          </div>
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-lg font-bold text-yellow-600">
                    ${service.financials.tipAmount.toFixed(2)}
                  </p>
                )}
              </div>
          ...
          {isEditing ? (
            <>
              <button
                ...
                className="text-red-600 hover:text-red-800 flex items-center gap-1 text-sm font-bold"
              >
