...
import { X, Car, User, DollarSign, Calendar, FileText, Trash2, Save, Edit } from 'lucide-react';
...
export default function ServiceDetailsModal({ service, onClose }: Props) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Lógica de recálculo financiero
      const oldRate = service.financials.commissionRate || 0.40; // Usar el % original
      ...
      onClose(); // Cerramos para refrescar datos en tabla
    } catch (error) {
      console.error(error);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                {isEditing ? (
                    <input 
                        type="number"
                        className="w-full p-2 border rounded font-bold text-lg bg-yellow-50 border-yellow-200"
                        value={formData.tipAmount}
                        onChange={e => setFormData({...formData, tipAmount: parseFloat(e.target.value) || 0})}
                    />
                ) : (
                    <p className="text-lg font-bold text-yellow-700">${service.financials.tipAmount.toFixed(2)}</p>
                )}
              </div>
            </div>
            {isEditing && (
                <p className="px-4 pb-2 text-xs text-red-500 italic">
                    * Al cambiar el precio total, la comisión del lavador se recalculará automáticamente basada en el % original.
                </p>
            )}
          </div>
        ...
        <div className="p-4 border-t bg-gray-50 flex justify-between items-center">
            {isEditing ? (
                ...
                <>
                    <div className="text-sm text-gray-400">Registrado: {dateStr}</div>
                    <div className="flex gap-2">
                        <button 
